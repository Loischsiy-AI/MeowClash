...
fun Service.createMeowClashNotificationBuilder(): Deferred<NotificationCompat.Builder> =
    CoroutineScope(Dispatchers.Main).async {
        val defaultComponent = ComponentName(packageName, "com.follow.clash.MainActivity")
        val lightComponent = ComponentName(packageName, "com.follow.clash.MainActivityLight")
        ...
        val pendingIntent = PendingIntent.getActivity(this@createMeowClashNotificationBuilder, 0, intent, flags)

        NotificationCompat.Builder(this@createMeowClashNotificationBuilder, GlobalState.NOTIFICATION_CHANNEL).apply {
...
fun Service.ensureNotificationChannel() {
    ...
    if (channel == null || channel.importance != NotificationManager.IMPORTANCE_HIGH) {
        manager?.createNotificationChannel(
            NotificationChannel(GlobalState.NOTIFICATION_CHANNEL, "MeowClash Service", NotificationManager.IMPORTANCE_HIGH)
        )
    }
}
