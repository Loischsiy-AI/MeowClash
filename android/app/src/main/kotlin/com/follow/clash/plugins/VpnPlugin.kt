...
import com.follow.clash.modules.SuspendModule
import com.follow.clash.services.BaseServiceInterface
import com.follow.clash.services.MeowClashService
import com.follow.clash.services.MeowClashVpnService
...
data object VpnPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
        ...
        scope.launch {
            try {
                val prepareIntent = try {
                    android.net.VpnService.prepare(MeowClashApplication.getAppContext())
                } catch (e: Exception) {
                    null
                }
    ...
    private suspend fun performStartCore(
        ...
        com.follow.clash.core.Core.startTun(
            fd = fd ?: 0,
        ...
        GlobalState.runLock.withLock {
            ...
            if (currentOptions.dozeSuspend) {
                ...
                suspendModule = SuspendModule(MeowClashApplication.getAppContext())
    ...
    fun handleStop(force: Boolean = false) {
        ...
        if (shouldForceStop) {
            context.stopService(Intent(context, MeowClashVpnService::class.java))
            context.stopService(Intent(context, MeowClashService::class.java))
        }
    ...
    private fun bindService() {
        ...
        try {
            ...
            val intent = Intent(
                ...
                if (options?.enable == true) MeowClashVpnService::class.java else MeowClashService::class.java
            )
            val res = MeowClashApplication.getAppContext().bindService(intent, connection, Context.BIND_AUTO_CREATE)
            if (!res) {
                isBinding.set(false)
                bindTimeoutJob?.cancel()
                bindTimeoutJob = null
